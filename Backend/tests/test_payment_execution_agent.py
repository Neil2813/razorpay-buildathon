...
from app.db.database import init_db, load_transaction_checkpoint
...
class PaymentIdempotencyTests(unittest.TestCase):
    ...
    def test_key_is_durable_before_gateway_call(self):
        ...
        try:
            state = new_transaction_state(tenant_id="tenant", session_id="session", user_message="buy")
            state.update({"guardrail_passed": True, "buyer_approved": True, "chosen_product": {"product_id": "sku-1", "price": 100.0}})
            ledger = SQLiteLedger()
            gateway = LostResponseGateway()
    ...
    def test_resume_uses_checkpointed_key_and_skips_completed_agents(self):
        ...
        try:
            ...
            with self.assertRaises(SystemExit):
                run_transaction(tenant_id="tenant", session_id="resume-session", user_message="Men blue shirt size M rating 4 stars between ₹50 and ₹1000", catalog=catalog, guardrail_ceiling=1_000.0, transaction=transaction, gateway=crashed_gateway, ledger=ledger, autonomy_mode="autonomous", buyer_approved=True)

            resumed_gateway = SuccessfulGateway()
            resumed = run_transaction(tenant_id="tenant", session_id="resume-session", user_message="ignored on resume", catalog=catalog, guardrail_ceiling=1_000.0, transaction=transaction, gateway=resumed_gateway, ledger=ledger, autonomy_mode="autonomous", buyer_approved=True)
